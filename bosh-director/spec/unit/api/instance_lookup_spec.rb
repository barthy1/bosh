...
module Bosh::Director
  module Api
    describe InstanceLookup do
      ...
      let!(:instance1) { Models::Instance.make(deployment: deployment, job: job_name, index: job_index1) }
      ...
      describe '.by_filter' do
        ...
        it 'finds only instances that are not excluded' do
          expect(instance_lookup.by_filter({job: instance.job}, {id: instance1.id})).to eq([instance])
        end
      ...
      describe '.find_all' do
        it 'pulls all instances' do
          expect(instance_lookup.find_all).to eq [instance, instance1]
        end
      end
